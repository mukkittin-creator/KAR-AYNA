# KÂR-AYNA
<img width="3435" height="1285" alt="Karşılaştırma Menüsü_Ana_Ekran" src="https://github.com/user-attachments/assets/80a1aa2c-1651-4040-b127-f7bef02796ce" />
<img width="3430" height="1290" alt="Metin Analizi_Sorgu_Ekranı" src="https://github.com/user-attachments/assets/6063bc4f-99aa-4fe3-8830-a7a4220282b1" />

## Proje Hakkında
KÂR-AYNA, Mukkittin tarafından geliştirilen ve katılımcı bankacılık kampanyalarının daha güvenli, daha şeffaf ve daha mevzuata uygun şekilde değerlendirilmesini amaçlayan bir yerel ilkeli analitik platformdur. Proje, metin tabanlı kampanya içeriklerini yapılandırılmış ürün çıkarımlarına dönüştürür; terminoloji kontrolü, açıklanabilir karşılaştırma ve karar destek akışı sunar.

## Milli Teknoloji Hamlesi Vizyonu
KÂR-AYNA, Türkiye'nin dijital bağımsızlık ve milli teknoloji hamlesi hedefleriyle uyumlu bir yaklaşım sunar. Sistem, veri işleme ve karar desteği süreçlerini dışa bağımlılık olmaksızın, yerel ortamda ve kontrol edilebilir bir mimariyle yürütmeyi hedefler. Böylece kurumların, güvenilir, egemen ve açıklanabilir yapay zeka kapasitesiyle daha güçlü içgörüler üretmesi amaçlanır.

## Temel Özellikler
- Yerel ve bağımsız çalışma modeli
- Katılımcı bankacılık terimleri için mevzuata uygunluk denetimi
- Açıklanabilir ürün karşılaştırma akışı
- Türkçe ve İngilizce destekli kullanıcı arayüzü
- Kurumsal taraflar için hızlı inceleme ve değerlendirme imkânı

## Kurulum
Aşağıdaki adımları izleyerek projeyi yerelde çalıştırabilirsiniz:

1. Proje dizinine gidin.
2. Bağımlılıkları yükleyin:
   - pip install -r requirements.txt
3. Uygulamayı başlatın:
   - streamlit run dashboard.py

## Çalıştırma
Kurulum tamamlandıktan sonra aşağıdaki komutla dashboard arayüzünü açabilirsiniz:

- streamlit run dashboard.py

Bu komut, kampanya metinlerini analiz edebileceğiniz, karşılaştırma yapabileceğiniz ve terminoloji kontrolü gerçekleştirebileceğiniz bir kullanıcı arayüzü başlatır.

## Lisans
Bu proje Apache Lisansı 2.0 kapsamında dağıtılmaktadır.
